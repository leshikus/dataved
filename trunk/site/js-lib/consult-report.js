var triples = [
  ...
  [
    ...
    'Помощь в организации взаимодействия с rian.ru и ВГТРК при проведении распределенных региональных мероприятий',
  ...
  [
    ...
    'Консультации по методикам и критериям, позволяющих на основании статистических данных, отражаемых в системе делопроизводства Lotus, установленной в Правительстве региона и подведомственных ему организаций, оценивать эффективность работы с документами пользователей указанной системы',
...
function generateAllTopics() {
  ...
  for (index = 0; index < triples.length; ) {
	...
	for (i = 0; i < consultees.length; j++) {
	  ...
	  if (!ALL_TOPICS[consultee]) allTopics[consultee] = [];
...
function generate(element) {
  ...
  element.value = '';
  var t = NUM_TOTAL;
  var consultees = getKeys(ALL_TOPICS);
  

  while (t > 0) {
    var consultee = getRandomElement(consultees);
	var topics;
